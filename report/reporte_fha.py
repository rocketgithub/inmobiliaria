...
class ReporteFha(models.AbstractModel):
    ...
    def cuota_nivelada(self,data):
        ...
        for i in range(1,data['cantidad_cuotas_enganche'] + 1):
            pago_capital = 0#numpy_financial.ppmt((data['tasa_interes_conjunta']/100) / 12,i,data['cantidad_cuotas_enganche'],-monto_financiar)
            pago_interes = 0#numpy_financial.ipmt((data['tasa_interes_conjunta']/100) / 12,i,data['cantidad_cuotas_enganche'],-monto_financiar)
